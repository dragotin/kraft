...
* Efficient creation of offers, invoices and similar documents.
...
Each document has to have a unique identifier that identifies the document. There must not be two documents in Kraft with the same identifier.

The format of the identifier is configurable in Kraft. For that, Kraft has a concept of so called number cycles.
Number cycles are used to define the form of the *document number* which is printed on every document.

All documents of a certain type have identifiers taken out of one number cycle. Each document type has a number cycle assigned. Different document types can use the same number cycle to generate ids from. That way, for example the document types Invoice and Final Invoice can use numbers from the same number cycle, even though they are different document types in Kraft.
Number cycles are identified by their name.
Users can create new number cycles and edit them clicking on the button btn:[Edit Number Cycles...]
...
In addition to the counter, more information can be added to the template to form a useful number. Examples are a constant text or parts of the date.

The default numbercycle delivered with Kraft contains the year, the month and a serial number. That way you can compare offers or orders from last year with this year or the month April of last year with the month April of this year and in this way get an impression from the results of your business.
...
Both `%i` and `%n` are numeric values. They can also be padded with leading zeros. The length is defined by the number of i's or n's put into the template. For example, if a daily counter with length of three and leading zeros is desired, `%nnn` has to be put into the template. This works up to a length of six characters.
____
NOTE:
The "design" of the numbercycles and document numbers is very important. With the flexible system of templating Kraft can not prevent invalid number cycles. It is the responsibility of the user.
____
...
4. *alternating*: For this, the watermark PDF needs to have three pages. The first page is merged to the first page of the result document, the second and third page are alternately merged to subsequent pages.
5. *different first and last page*: the PDF needs to have three pages. The first page is merged to the first page of the result document, the third page to the last, and the second to all pages between first and last page.
...
In many countries there are two kinds of VAT for sold products.
...
A list of units of measurement is maintained in Kraft. In Kraft Configuration Dialog reachable through the Settings menu you can edit and customize items already in the
list, and you can also add new items to the list.
...
____
WARNING:
If you made the choice to use the information from KaddressBook then the
information from a later manual entry is ignored.
____
...
Kraft supports so called Catalogs in which templates for document items are kept. With the catalogs creating documents can be significantly accelerated in the day to day business. When creating new documents, the items templates from the catalogs can be easily selected and moved over to the document.
...
Both catalogs can have chapters and sub-chapters to organize your
...
After this we add also in the folder 'Wood' an item for a 'support pole' for later use with its price.
...
We fill here in a number of work steps with their time:
...
We navigate to the folder 'wood' where we select the 'support pole'.
...
Kraft supports text templates also for the header- and footer-text of documents. Each document type has its own set of text templates. To access them, open a document of a certain document type in edit mode and use the btn:[show templates] to make the templates visible. In both the header- and footer-section the stored templates become visible.
...
- `DATE_ADD_DAYS(<amount_days>)`: Returns the date of the document plus the number of days specified as parameter to the macro.
- `ITEM_COUNT_WITH_TAG(<tag>)`: Returns the amount of items tagged with the tag.
- `NETTO_SUM_PER_TAG(<tag>)`: Returns the net sum of all items that are tagged with the named tag.
- `BRUTTO_SUM_PER_TAG(<tag>)`: Returns the gross sum of all items that are tagged with the named tag.
...
The document [creation wizard] opens.
...
Select on the new window the name and address of the client.

(if the name and address are not there, click then on btn:[new contact] or on
btn:[edit contact] if you want to edit the contact)

Click on btn:[OK].

Now the document [items] window opens.
...
The right tab changes and now shows the earlier made templates, we select
in the group Work, the subgroup Plant tree and click then on the button
with the left-pointing arrow at the bottom.
...
The window closes and we go back to the main window.
...
____
WARNING:
Remember that in the catalog we can add a profit on the price of the
...
A PDF will now be made from the invoice which we can print on paper or
send by email to the client.

After this we are going to create an offer for some work in a garden.
...
Beside this, we have seen that there is a lifeless tree, which we will offer to remove as extra work.
...
subchapter `Broadleaf trees` in which we select the `apple tree` which we made
earlier.
...
We want the client to be able to make a choice from an apple, a pear tree and the liguster.

Therefore we are also going to add a pear tree manually.
...
fill in the number of the special trees that we have delivered and the price of them.
...
We do this again but then for the liguster.
...
We select the upper button with the page after which a context-menu opens with the items:
...
We see now that the price of the pear tree, the liguster and the removal
of the tree is not used for the total price. When we are happy with the
result, we can click on the button btn:[close] after which we click on the
button btn:[Print Document] for making a PDF that we can print out or send to
the client.

After your first invoice, your first offer is now also ready for
sending.
...
We can make the "Acceptance of order" in the same way as we made the invoice or the offer by selecting each item and correcting the number of delivery.
...
image:followup_1.png[Followup document,float="right"]

We select here "Acceptance of order".
We have now a copy of the offer as an Acceptance of order
(do not forget to adapt Alternative Delivery items and on demand items.)

image:followup_2.png[Followup document,float="right"]
...
However, in some situations the KAddressBook Integration can not be set up. In those cases, Kraft can also read
an address collection of a local directory. The directory can be set in the Kraft config file in group `[KraftV2]`, key `KraftV2AddressDir`. If not explicitly set, it defaults
...
To create PDF output documents, the document data that was edited in the Kraft app is filled into a template. The template defines how the output document looks like, i.e. by font settings, placing of elements and such.
...
Each document type in Kraft can have its own template that is used to create a PDF. Which one can be set in the Settings dialog for document types.

==== WeasyPrint Documents

NOTE: Kraft still ships with the old ReportLab based converter, but that will be deprecated in the future. The WeasyPrint based system is the future. It is recommended to port existing templates.
...
The syntax is based on the Django syntax for templates described in the https://docs.djangoproject.com/en/3.1/topics/templates/[docs].

include::templatevars.md[]

==== Adding a Logo

On the PDF output it is nice to have a logo image included, if the documents should not be printed on a prepared stationery that comes with a printed logo already.
...
[[edit_template]]
 ...
 [Ctrl]+[E]
 Opens a window where you can add, edit or translate the tags (like work,
 material, plants or discounts).
[[redo]]
 [Settings]>[Redo initial setup]
 [Ctrl+R]
 Repeats the initial setup. After this, a restart of Kraft is required.
[[toolbars]]
 [Settings]>[Shown toolbars]
...
Kraft can be used collaboratively in a distributed environment. That means that multiple users work on their desktops with their own Kraft instance on the same data.
...
There are different ways how to share that directory, i.e. NFS or SMB storages. It is important that both users from both machines can list and access the files. The main user needs read and write access, read only users only need read access to the files.
...
To use the XRechnung Export productively, a little manual work is still needed in Kraft. Kraft creates the XML file based on a template, very similar to the normal PDF documents. That means that it loads a template that contains static elements (i.e. the company address) that do not change between different invoices. The dynamic elements (customer data, items etc.) are filled into the template during the generation step.

In order to generate correct XRechnung files for the specific company that is using Kraft, the admin has to adapt the template file manually to the company's needs. Note that this has only to be done once and should be easy for a person with a bit of computer experience (Basic knowledge about XML appreciated!).
...
Read carefully through the file without being scared off by the XML format. All user strings (i.e. company name, address and such) are user specific and should be replaced accordingly. Find https://www.xoev.de/xrechnung-16828[details about the format] here to better understand the meaning of the fields.

Make sure to not disturb the proper XML format and do not change places where the template format `{{ template_name }}` is used.

Once the file is adapted to the needs, open the Settings dialog of Kraft and insert the filename into the entry field for the XRechnung Template File on the Document Defaults page.
...
After those steps, the btn:[Export XRechung] menu item will let you open a dialog to add a few additional parameters for the XRechnung that Kraft does not have by default, and lets you pick a filename where to save the XRechnung invoice file to.
...
Program and documentation copyright 2004–2026 Klaas Freitag

Documentation copyright 2020-2026 Ronald Stroethoff
