...
Kraft supports two types of counter: One (variable `%i`) is incremented globally for every new document of a certain type. The other (variable `%n`) is reset every day and starts with 1 again on every new day.
...
The default numbercycle which is delivered with Kraft contains the year, the month and a serialnumber, this makes it that you can compare offers or orders from last year with this year or the month April of last year with the month April of this year and get on this way an impression from the results of your business.
...
| `%n`            | the unique day counter (combine with date)
